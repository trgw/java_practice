...
public class Enemies extends GameObject {
    ...
    public void newEnemy(int xSize, int timeInc) {
        int r = 0;
        for (int a = 0; a < num; a++) {
            if (r == 0 & Open[a] == 0) {
                Open[a] = 1;
                r = 1;
                Speed[a] = (int) (( Math.random() * 5) + 1); // スピード 1～5 の中でランダム
                Type[a]  = (int) (( Math.random() * 2) + 1); // タイプ　 1 or 2
                Slope[a] = (int) (((Math.random() * 3) + 1) - 1); // スロープ 0～3 の中でランダム
    ...
    public void draw(Graphics g, MyCanvas canvas) {
        for (int i = 0; i < num; i++) {
            if (Open[i] == 2) {
                g.drawImage(exImage, X[i], Y[i], canvas);
            }
            if (Open[i] == 1) {
                if (Type[i] == 1) {
                    g.drawImage(image1, X[i], Y[i], canvas);
                } else {
                    g.drawImage(image2, X[i], Y[i], canvas);
                }
            }
        }
    }
